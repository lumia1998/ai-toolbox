...
- Windows MSI/WiX 不接受带文本的 SemVer prerelease。需要保留 `1.7.0-beta.2` 这类应用版本时，通过 `bundle.windows.wix.version` 单独提供 `major.minor.patch.build` 数字版本，例如 `1.7.0.2`；不要为了 MSI 改掉应用和发布标签的 beta 语义。
