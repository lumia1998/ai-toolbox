...
- Custom provider Gateway protocol selection is stored in provider `meta.apiFormat`, not in Gemini CLI runtime `settingsConfig`. The default is Gemini Native; selecting Claude/Anthropic only changes Gateway target protocol and must preserve existing billing meta.
...
- Provider mode can only be selected while adding a blank new provider. Copying a provider still creates a new provider record, but the mode selector must be hidden and the source provider `category` must be preserved. Editing a saved provider must also hide the mode selector and preserve the existing `category`.
